...
from adapters.alex_working_memory_adapter import AlexWorkingMemoryAdapter

ADAPTERS = {
    "secure_reference": SecureReferenceAdapter,
    "vulnerable_reference": VulnerableReferenceAdapter,
    "alex_working_memory": AlexWorkingMemoryAdapter,
}
...
def main():
    ...
    run_p.add_argument("--db-path", default=None, help="Nur fuer alex_working_memory: Pfad zur (Kopie der) ghost.db")

    args = parser.parse_args()

    if args.command == "run":
        adapter_cls = ADAPTERS[args.adapter]
        adapter = adapter_cls(args.db_path) if args.adapter == "alex_working_memory" else adapter_cls()
